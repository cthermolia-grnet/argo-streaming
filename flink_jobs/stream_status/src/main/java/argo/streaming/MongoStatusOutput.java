...
public class MongoStatusOutput implements OutputFormat<String> {
	...
	public StatusEvent jsonToStatusEvent(JsonObject jRoot) {
		...
		String summary = extractJson("summary",jRoot);
                String url=extractJson("url", jRoot);
		return new StatusEvent(rep,tp,dt,eGroup,service,hostname,metric,status,monHost,tsm,tsp,prevTs,prevStatus,repeat,summary,message,url);
	}
