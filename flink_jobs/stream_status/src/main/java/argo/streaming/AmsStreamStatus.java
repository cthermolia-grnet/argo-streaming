 ...
 * kafka, fs) job required cli parameters
 ...
 * --hbase.namespace : hbase namespace --hbase.table : hbase table name 
...
public class AmsStreamStatus {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple3<String, String, MetricData>> groupMdata = metricAMS.connect(syncA)
     ...
     */
    private static class MetricDataWithGroup extends RichCoFlatMapFunction<String, String, Tuple3<String, String, MetricData>> {
         ...
         */
        @Override
        public void flatMap1(String value, Collector<Tuple3<String, String, MetricData>> out)
            ...
            String service = item.getService().toString();
            String hostname = item.getHostname().toString();

            ArrayList<String> groups = egp.getGroup(hostname, service);

            //System.out.println(egp.getList());
            for (String groupItem : groups) {
                String url = egp.getTagUrl(groupItem, hostname, service);
                Tuple3<String, String, MetricData> curItem = new Tuple3<String, String, MetricData>();
        ...
        public void flatMap2(String value, Collector<Tuple3<String, String, MetricData>> out)
                throws IOException, ParseException {
     ...
     */
    private static class StatusMap extends RichCoFlatMapFunction<Tuple3<String, String, MetricData>, String, String> {
         ...
         */
        @Override
        public void flatMap1(Tuple3<String, String, MetricData> value, Collector<String> out)
                throws IOException, ParseException {

            MetricData item = value.f2;
            String group = value.f0;
            String url = value.f1;

            String service = null, hostname = null, metric = null, status = null, tsMon = null, monHost = null, message = null, summary = null;

            if (item.getService() != null) {

                service = item.getService().toString();
            }
            if (item.getHostname() != null) {
                hostname = item.getHostname().toString();
            }
            if (item.getMetric() != null) {
                metric = item.getMetric().toString();
            }
            if (item.getStatus() != null) {
                status = item.getStatus().toString();
            }
            if (item.getTimestamp() != null) {
                tsMon = item.getTimestamp().toString();
            }
            if (item.getHostname() != null) {
                monHost = item.getMonitoringHost().toString();
            }
            if (item.getMessage() != null) {
                message = item.getMessage().toString();
            }
            if (item.getSummary() != null) {
                summary = item.getSummary().toString();
            }

//                        
//			String service = item.getService().toString();
//			String hostname = item.getHostname().toString();
//			String metric = item.getMetric().toString();
//			String status = item.getStatus().toString();
//			String tsMon = item.getTimestamp().toString();
//			String monHost = item.getMonitoringHost().toString();
...
//                                       
            ...
            if (config.daily
                    && sm.hasDayChanged(sm.getTsLatest(), tsMon)) {
            ...
            ArrayList<String> events = sm.setStatus(group, service, hostname, metric, status, monHost, tsMon, summary, message, url);
